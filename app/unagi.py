...
# copy gzip detection from https://stackoverflow.com/questions/42757283/seqio-parse-on-a-fasta-gz
import gzip
...
def main(argv):
    ...
    parser.add_argument('-o', '--output', help='Output path. This should be the path to output the result files to.',
                        required=True)
    parser.add_argument('-g', '--genome', help='Genome File. This should be a valid genome file for minimap.',
                        required=True)
    parser.add_argument('-s', '--stranded',
                        help='Stranded. Should be selected if the input file contains reads that are already stranded.',
                        action='store_true', required=False)
    parser.add_argument('-V', '--verbose', help='Verbose. Displays additional information while running.',
                        action='store_true', required=False)
    parser.add_argument('-S', '--silent', help='Runs silently (no console output), cancels verbose.',
                        action='store_true', required=False)
        ...
        with open(sourceFile, "r") as dashList:
            for line in dashList:
                ...
                if int(currentDashSite["coverage"]) > int(config["min_" + dash + "dash_threshold"]):
                    ...
                    if lastDashSite is None or int(currentDashSite["position"]) - int(lastDashSite["position"]) < int(
                            config["min_" + dash + "dash_peak_separation"]):
            ...
            for line in rawIsomophs:
                ...
                for exon in exons:
                    ...
                    if len(sequence) < int(config["min_exon_length"]) and (
                            polyA in sequence or polyT in sequence or sequence.count('A') / len(sequence) > float(
                            config["max_single_base_to_length_ratio"]) or sequence.count('T') / len(sequence) > float(
                            config["max_single_base_to_length_ratio"])):
                        pass
                    else:
                        if start is None:
                            start = exon[0]
...
def findBestIsoform(sourceFile, outputFile):
    """Creates a file containing only the most frequent isomorphs from a file containing a list of them"""
        ...
        with open(outputFile, "w") as outFile:
            ...
            for line in inFile:
                ...
                if lastline is not None and (
                        ...
                        int(lastline[5]) - int(currentline[5])) < int(config["max_splice_difference"])):
